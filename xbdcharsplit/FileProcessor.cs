...
namespace xbdcharsplit
{
    public class FileProcessor
    {
        ...
        public bool ProcessFile(string inputFile, out Characters characters)
        {
            ...
            try
            {
                ...
                using (BinaryReader b = new BinaryReader(File.Open(inputFile, FileMode.Open, FileAccess.Read, FileShare.Read)))
                {
                    ...
                    if (true)
                    {
                        ...
                        Console.WriteLine($"Reading Int32 at [{b.BaseStream.Position}]");
                        numberOfCharacters = b.ReadInt32();
                        Console.WriteLine($"  Number of characters is {numberOfCharacters}");
                    ...
                    for (int i = 1; i <= numberOfCharacters; i++)
                    {
                        ...
                        if (true)
                        {
                            try
                            {
                                ...
                                Console.WriteLine($"Character name length is {charNameLength}");

                                character.CharacterName = new string(b.ReadChars(charNameLength).ToArray());
                                Console.WriteLine($"  Character name is {character.CharacterName}");
                            }
                            catch (Exception ex)
                            {
                                return false;
                        ...
                        while (b.BaseStream.Position < fileLength)
                        {
                            ...
                            {
                                ProcessFaces(b, character.FacesList);
                            }
                            else if (IsVertices(b))
                            {
                                ProcessVertices(b, character.VerticesList);
                            }
                            else
                            {
                                try
        ...
        private bool IsNewCharacter(BinaryReader b)
        {
        ...
        {
            // seek forward 41 to get to length
            //b.BaseStream.Seek(41, SeekOrigin.Current);
            ...
            //Console.WriteLine($"    Reading {verticesLength * 8} bytes at {b.BaseStream.Position}...");
            //byte[] facesBytes = b.ReadBytes(verticesLength * 8);

            for (var thisVertex = 0; thisVertex < verticesLength; thisVertex++)
            {
                //var index = thisVertex * 8;
                //Vertex vertex = new Vertex()
                //{
                //    //Index1 = BitConverter.ToUInt16(new byte[2] { facesBytes[index + 0], facesBytes[index + 1] }),
                //    //Index2 = BitConverter.ToUInt16(new byte[2] { facesBytes[index + 2], facesBytes[index + 3] }),
                //    //Index3 = BitConverter.ToUInt16(new byte[2] { facesBytes[index + 4], facesBytes[index + 5] }),
                //    //X = BitConverter.ToUInt16(facesBytes, index),
                //    //Y = BitConverter.ToUInt16(facesBytes, index + 2),
                //    //Z = BitConverter.ToUInt16(facesBytes, index + 4)
                //    //Half.
                //    X = Half.ToHalf(facesBytes, index),
                //    Y = Half.ToHalf(facesBytes, index + 2),
                //    Z = Half.ToHalf(facesBytes, index + 4)
                //};
